...
router.post("/login", async (req: Request, res: Response) => {
    ...
    try {
        const result = await ws.request<{ success: boolean; token?: string; username?: string }>(id_, JSON.stringify({
            username,
            password,
        }))
        ...
        if (result.success && result.token) {
            ...
            res.cookie("username", result.username ?? username, { httpOnly: true, sameSite: "lax" })
            return res.redirect("/")
        }
